...
const state = { items: [], processing: false, phase: '', batchTotal: 0, uploaded: 0, completed: 0, cloudStartedAt: 0 };
...
async function addFiles(files) {
  ...
  try {
    const job = await api('/api/jobs', { method: 'POST', json: { count: accepted.length } });
...
async function pollJob(job, items) {
  ...
  while (true) {
    ...
    if (result.status === 'error') throw new Error(result.error || '云端抠图失败');
...
function detectScreenshotCrop(bitmap) {
  ...
  return ratio > .004 && ratio < .2 ? Math.round(bitmap.height * .66) : bitmap.height;
}
